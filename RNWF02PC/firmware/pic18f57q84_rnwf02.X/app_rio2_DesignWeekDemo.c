...
static volatile bool waituntilSW0Press = 1;
...
void APP_RIO2_Tasks(void) {
    ...
    temperature = (((temperatureValue * gain) / 256) + offset) / 10;
...
#define PUBSTATESNUM_TOTAL 6
        ...
        case APP_MQTT_STATE_PUBLISH:
        {
            ...
            switch (pubState) {
                ...
                {
                    sprintf(buffer, "AT+MQTTPUB=0,0,0,\""PUB_TOPIC_TELEMETRY"\",\""PUB_AZURE_PAYLOAD_TELEMETRY_COUNTER"\"\r\n", thingID, pubCount++);
                    pubState++;
                    break;
                }
