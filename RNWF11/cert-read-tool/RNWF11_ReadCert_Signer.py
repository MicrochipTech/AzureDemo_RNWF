...
class ReadSignerCert:
  ...
  def evt_read_certificate(self) :
    ...
    if os.name == 'posix' :
      cmd_line = "mv Cert.pem " + cn + "_signer.pem"
    else:
      cmd_line = "rename Cert.pem " + cn + "_signer.pem"
    print(cmd_line)
  ...
  def runApp(self):
    ...
    if self.app_state == 0:  # start of application
      print("\r\n--------------------------------------------------------------------------------")
      print("Reading RNWF11 Signer Certificate")
      print("--------------------------------------------------------------------------------\r\n")
      self.app_state = 1
...
certificate = ReadSignerCert(PORT, RNWF11_BAUDRATE, False)
