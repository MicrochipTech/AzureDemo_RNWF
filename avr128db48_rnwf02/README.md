...
NOTE: For evaluation purposes, an example chain of trust has been provided in this [sub-folder](./cert-chain-gen-tool/). If you are not interested in going through the detailed steps of creating your own chain of trust at this time, feel free to jump directly to Step 2.2 below and use the provided set of example files.

#### 2.1 Generate the files for your certificates & keys

Click [here](./cert-chain-gen-scripts/) and follow the instructions to use Git Bash shell scripts to generate your chain of trust. During execution of the scripts, you will be asked to provide names for the Root CA, Signer (Subordinate/Intermediate) CA, and the Common Name (CN) for the device.

#### 2.2 Upload the Device's Certificate & Private Key into the RNWF02PC Module
...
* Launch a Terminal Emulator window and connect to the Virtual COM port corresponding to the PC's USB-to-UART serial bridge connection to the RNWF02PC Add On Board. Create a new serial port connection at 230400 baud. Hit the [RETURN] key once or twice to see the command prompt generated by the RNWF02PC module.

* Issue the following command in the terminal to see a list of all certificates currently stored in the RNWF02PC module:

    ```bash
    AT+FS=2,1
    ```

* Issue the following command to configure the module to use `myDevice_cert` for the device certificate during the Transport Layer Security (TLS) authentication phase:
...
* Issue the following command to configure the module to use `myDevice_key` for the device key during the Transport Layer Security (TLS) authentication phase:
...
* Upon successful completion of uploading the device certificate & key files to the RNWF02PC module (and configuring which ones the module should use during the TLS authentication phase), the USB-to-UART converter can be disconnected now.
